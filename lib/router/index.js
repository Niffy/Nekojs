...
module.exports = class Router {
  ...
  use(route, fn) {
    ...
    // or we throw
    if ('[object Function]' !== utils.objType(fn)) {
      throw new Error('No callback given for route: ' + route)
    }
  ...
  create(route, fn, method) {
    // Default route to / if no route was given.
    if ('string' !== typeof route) {
      throw new Error('Router.create() - Route must be a string:' + route)
    }
   ...
   */
  handle (req, res, done) {
    debug.info('Dispatching route: %s', req.url)
    ...
    function next (err) {
      ...
      if (!handler) {
        return utils.defer(done, req, res)
      }

      // we also need to check if the handler needs
      // to be exectuted
      if (!self.shouldRun(path, handler.route)) {
        return next(err)
      }
   ...
   *  @api private
   */
  shouldRun(path, route) {
    let c1 = route === '*'
    let c2 = (path.toLowerCase() === route.toLowerCase())
    let c3 = (path.startsWith(route) && route.length > 1)
    let c4 = (c1 || (c2 || c3)) // for the lolz
    return c4
  }
